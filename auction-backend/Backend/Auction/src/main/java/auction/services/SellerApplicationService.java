    ...
    public SellerApplication createApplication(SellerApplicationRO applicationRO, HttpSession session) {
        try {
            ...
            if (loggedInUser == null) {
                throw new ServiceException("User is not logged in", new RuntimeException("No user found in session"));
            }

            SellerApplication application = new SellerApplication();
            application.setUser(loggedInUser);
            application.setStatus(applicationRO.getStatus() != null ? applicationRO.getStatus() : ApplicationStatus.PENDING);
